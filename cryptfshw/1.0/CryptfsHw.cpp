...
int CryptfsHw::cryptfs_hw_create_key(enum cryptfs_hw_key_management_usage_type usage,
...
int CryptfsHw::__cryptfs_hw_wipe_clear_key(enum cryptfs_hw_key_management_usage_type usage,
                                           int wipe_key_flag) {
...
int CryptfsHw::cryptfs_hw_wipe_key(enum cryptfs_hw_key_management_usage_type usage) {
    int32_t ret;
...
int CryptfsHw::cryptfs_hw_clear_key(enum cryptfs_hw_key_management_usage_type usage) {
    int32_t ret;
...
int CryptfsHw::cryptfs_hw_update_key(enum cryptfs_hw_key_management_usage_type usage,
...
unsigned char* CryptfsHw::get_tmp_passwd(const char* passwd) {
    int passwd_len = 0;
        ...
        if (tmp_passwd) {
            if (operation == UPDATE_HW_DISK_ENC_KEY) {
                if (tmp_currentpasswd) {
                   err = cryptfs_hw_update_key(map_usage(CRYPTFS_HW_KM_USAGE_DISK_ENCRYPTION),
...
Return<int32_t> CryptfsHw::setKey(const hidl_string& passwd, const hidl_string& enc_mode) {
    return set_key(NULL, passwd.c_str(), enc_mode.c_str(), SET_HW_DISK_ENC_KEY);
}

Return<int32_t> CryptfsHw::updateKey(const hidl_string& oldpw, const hidl_string& newpw,
                                     const hidl_string& enc_mode) {
    return set_key(oldpw.c_str(), newpw.c_str(), enc_mode.c_str(), UPDATE_HW_DISK_ENC_KEY);
}
